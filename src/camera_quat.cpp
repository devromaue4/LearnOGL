...
void CameraQuat::SetProjParams(float fAspect, float fovy, float zNear, float zFar) {
...
void CameraQuat::OnMouse(float x, float y, bool constrainPitch) {
	float xpos = x, ypos = y;
...
void CameraQuat::OnKeyboard(unsigned int key/*, float deltaTime = 0*/) {
	switch (key) {
	case 61: //plus
		m_speed += 0.01f;
...
glm::mat4 CameraQuat::GetMatrixLH() {
...
glm::mat4 CameraQuat::GetMatrixRH() {
	const glm::vec3 f = glm::normalize(m_target);
